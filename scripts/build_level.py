    ...
    for row_idx, line in enumerate(lines):
        for col_idx, char in enumerate(line):
            ...
            if char == 'B':
                ...
                level_data['coin_boxes'].append({
                    'x': x, 'y': y, 'contents': 'coin'
                })
            elif char == '#':
                # Pipe - calculate height based on consecutive # vertically
                pipe_height = 1
                for check_row in range(row_idx + 1, min(row_idx + 4, len(lines))):
                    if (check_row < len(lines) and col_idx < len(lines[check_row])
                            and lines[check_row][col_idx] == '#'):
                        pipe_height += 1
                    else:
                        break
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description='Build Super Mario Bros levels from text layouts'
    )
    parser.add_argument(
        '--name', '-n',
        default='custom_level',
        help='Level name (default: custom_level)'
    )
    parser.add_argument(
        '--input', '-i',
        help='Input layout file (default: creates sample)'
    )
    parser.add_argument(
        '--output', '-o',
        default='data/levels',
    ...
    parser.add_argument(
        '--sample', '-s',
        action='store_true',
        help='Create sample layout file'
    )
